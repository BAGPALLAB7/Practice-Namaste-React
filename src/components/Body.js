    ...
    const searchedRestroList = restaurantList.filter((data) => { 
      return (
         data.info.cuisines
           .join(" ")
           .toLowerCase()
           .includes(searchData.toLowerCase()) || data.info.name.toLowerCase().includes(searchData.toLowerCase())
       );
    });
  ...
  return (
    <div className="body">
      <div className="flex py-4">
        ...
        <button className="bg-blue-300 mx-4 w-40 rounded-md" onClick={() => filterResto()}>
          Filer Restaurants
        </button>
      </div>
